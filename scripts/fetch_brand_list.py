#!/usr/bin/env python3
"""
fetch_brand_list.py - 공정거래위원회 가맹정보 브랜드 목록(FftcBrandRlsInfo2_Service) 전량 수집
연도 기준 플랫 페이지네이션이라 한 번에 전체 수집 가능.

출력: _rawdata/brand_list_raw.json
"""
...
OUT = os.path.join(ROOT, "_rawdata", "brand_list_raw.json")

SERVICE_KEY = os.environ.get("DATA_GO_KR_API_KEY") or "9490b1d34e92aa9e25b32a4cff1438fc7b9c71e5d332413916a391e867f61e86"
BASE = "https://apis.data.go.kr/1130000/FftcBrandRlsInfo2_Service/getBrandinfo"
...
def fetch_page(page_no, attempt=1):
    params = {
        ...
        "pageNo": page_no, "numOfRows": PAGE_SIZE, "jngBizCrtraYr": YEAR,
    }
    try:
...
def main():
    ...
    while True:
        ...
        if total_count is None:
            ...
            print(f"전체 브랜드 수({YEAR}년): {total_count}")
